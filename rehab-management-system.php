...
function arms_create_system_tables() {
    ...
    // Core Consolidated Inpatient Registry (Required for Unified 5-Tab Interface)
...
function arms_log_activity( $action_description ) {
    ...
    $ip_address = ! empty( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : '0.0.0.0';
...
function arms_main_router_page() {
    // Infrastructure routing table with precise inline raw Font Awesome Pro 5.15.0 SVGs
    $all_tabs = array(
    ...
    // Resolve fallback routing if target configuration mapping breaks
    if ( ! array_key_exists( $active_tab, $all_tabs ) ) {
        $active_tab = 'dashboard';
    }

    // High-level explicit security validation logic layer check
    ...
    <div id="arms-wrapper" class="rehab-management-system <?php echo $is_print_mode ? 'arms-print' : ''; ?>">
            ...
            <ul class="arms-left-tabs">
                ...
                foreach ( $all_tabs as $slug => $config ) : 
                    if ( ! arms_has_access( $config['roles'] ) ) {
                        continue; // Hidden completely from UI layout stack if active session fails capabilities verification
                    }
                    ...
                    <li>
                        <a class="<?php echo esc_attr( $active_class ); ?>" 
                           href="<?php echo esc_url( admin_url( 'admin.php?page=rehab_management_system&tab=' . $slug ) ); ?>">
                            <?php echo $config['svg']; // Render Ultra-Performance Raw SVG Inline ?>
        ...
        <div class="arms-right-box">
            <?php
            // Structural callback delivery routing mapping directly into sub-directory template loops
            switch ( $active_tab ) {
                case 'dashboard':
                    arms_dashboard_tab();
                    break;
